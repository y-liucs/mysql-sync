...
public class SyncService {
	...
	private void syncTable(SyncConnection syncConnection, TableConfig table) {
		...
		//2.同步新增
		syncInsert(syncConnection, table);
		//3.同步更新
		if (table.isUpdate()) {
			logger.debug(table.getTableName() + "开始验证是否有更新...");
			syncUpdate(syncConnection, table);
	...
	private void syncUpdate(SyncConnection syncConnection, TableConfig table) {
	...
	private void syncInsert(SyncConnection syncConnection, TableConfig table) {
		String tableName = table.getTableName();
